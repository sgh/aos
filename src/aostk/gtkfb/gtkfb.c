...
static void gtkfb_line(int x1, int y1, int x2, int y2, unsigned int color);
...
static void gtkfb_init(void) {
	...
	* or if we return FALSE in the "delete_event" callback. */
	...
	pixmap = gdk_pixmap_new(NULL, gtkfb_device.xsize, gtkfb_device.ysize, 24);
// 	img = gtk_image_new_from_pixmap (pixmap, NULL);
  ...
  gtk_widget_set_size_request (drawing_area, gtkfb_device.xsize, gtkfb_device.ysize);
...
// 	gtk_widget_show (img);
	...
	gdk_draw_rectangle(pixmap,drawing_area->style->bg_gc[GTK_WIDGET_STATE (drawing_area)], TRUE, 0,0, gtkfb_device.xsize, gtkfb_device.ysize);
...
static void gtkfb_putpixel(int x, int y, unsigned int color) {
	int dx,dy;
	GdkColor rgb;
